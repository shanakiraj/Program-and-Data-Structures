...
mergeSort:
	...
	mov 	rax, 0		;rax to 0
	cmp	rsi, rdx	;compare
	jge	merge_done	;jumps to end
	add	rax, rsi	;add start to rax
	add	rax, rdx	;add end to rax
	push	rsi		;save rsi
	push	rdx		;save rdx
	shr	rax, 1		;divide rax by 2 to get mid
	mov	rdx, rax	;move mid into end
	call	mergeSort  	;recursive mergesort call
	add	rdx, 1		;add one to mid or rdx
	mov	rsi, rdx	;move mid into rsi or start
	pop	rdx		;restore end into rdx
	call	mergeSort  	;recursively call mergesort
	mov	rcx, rdx	;moves end into rcx
	sub 	rsi, 1		;subtracts mid by 1
	mov	rdx, rsi	;moves mid into rdx
	pop	rsi		;restores start to rsi
	call	merge		;calls merge
